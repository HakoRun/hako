...
const APPLETS: &[&str] = &[
    "arch", "base64", "basename", "bunzip2", "bzcat", "cal", "cat", "chattr",
    "chgrp", "chmod", "chown", "chroot", "cksum", "clear", "cmp", "comm",
    "cp", "cpio", "cut", "date", "dd", "df", "dirname", "du", "echo", "env",
    "expand", "factor", "false", "file", "find", "flock", "fmt", "fold",
    "free", "getopt", "grep", "groups", "gunzip", "head", "help", "host",
    "hostname", "id", "install", "kill", "killall", "link", "ln", "logger",
    "logname", "ls", "lsattr", "md5sum", "mkdir", "mkfifo", "mknod",
    "mktemp", "mount", "mountpoint", "mv", "nice", "nl", "nohup", "nproc",
    "od", "paste", "patch", "pgrep", "pidof", "pkill", "printenv", "printf",
    "ps", "pwd", "readlink", "realpath", "renice", "rev", "rm", "rmdir",
    "sed", "seq", "sh", "sha1sum", "sha256sum", "sha512sum", "shred", "shuf",
    "sleep", "sort", "split", "stat", "strings", "su", "sync", "tac", "tail",
    "tar", "tee", "test", "time", "timeout", "top", "touch", "true", "tr",
    "tsort", "tty", "uname", "uniq", "unlink", "uptime", "uudecode",
    "uuencode", "uuidgen", "watch", "wc", "which", "who", "whoami", "xargs",
    "xxd", "yes", "zcat",
];
...
pub fn extract_rootfs(store: &dyn ChunkStore) -> io::Result<Hash> {
    ...
    root = scoped.write_file_meta(&root, "etc/resolv.conf", ETC_RESOLV_CONF, DEFAULT_FILE_MODE, 0)?;
...
mod tests {
    ...
    fn applets_all_resolve_to_toybox() {
        ...
        for applet in APPLETS {
            let path = format!("bin/{}", applet);
            let target = scoped.read_symlink(&root, &path).unwrap_or_else(|e| {
                panic!("applet {} not a symlink: {}", applet, e)
            });
